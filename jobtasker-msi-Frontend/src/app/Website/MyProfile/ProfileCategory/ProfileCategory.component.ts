...
@Component({
  selector: 'app-ProfileCategory',
  templateUrl: './ProfileCategory.component.html',
  styleUrls: ['./ProfileCategory.component.scss']
})
export class ProfileCategoryComponent implements OnInit {
  ...
  constructor(private ds : DataService,
    private ts : ToastrService) { 
      this.ds.getMyCategoryList().subscribe((resp: any) => {
        if (resp.success) {
          this.states = resp.data
        }
      })
      this.ds.getMyCategory().subscribe((resp:any) => {
        if (resp.success) {
  ...
  updateSkill() : void {
    if (this.perAddedSkills.length < 1) {
      this.ts.warning('Please add category to update');
      return;
    }

    let obj = {
      categoryList : this.perAddedSkills
    }

    this.ds.updateCategory(obj).subscribe((resp:any) => {
      if (resp.success) {
